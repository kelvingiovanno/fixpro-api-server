...
<div class="max-w-4xl min-h-[700px] mx-auto p-6 bg-white my-5 flex space-x-6">
    ...
    <form action="{{ route('settings.member.submit') }}" method="POST">
        <section class="w-3/4">
            ...
            <h3 class="text-lg font-normal mb-2 mt-7">Customize Member Data Fields</h3>
            <p class="text-sm text-gray-600 mb-4">
                Add what member information to collect, like email, phone, or other custom details for better management.
            </p>
                ...
                @foreach($form as $column)
                    <div class="flex items-center space-x-2">
                        <input
                            type="text"
                            name="forms[]"
                            value="{{ ucfirst(str_replace('_', ' ', $column)) }}"
                            class="flex-1 p-2 border border-gray-300 rounded text-sm"
                            readonly
                        />
                        <button type="button" onclick="removeField(this)">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-4 text-black" viewBox="0 0 448 512" fill="currentColor">
